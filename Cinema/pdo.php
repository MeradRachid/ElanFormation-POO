<!-- 
    ...
    Le bloc catch peut recevoir l'objet exception généré dans le bloc try, ce qui permet de récupérer des informations sur l'erreur, telles que son message et sa cause.
...
catch (PDOException $e) 
{
    ...
    // Le premier appel, $stmtCinema->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);, configure le mode d'erreur de PDO. 
    // On configure donc PDO pour qu'il lance une exception de type PDOException en cas d'erreur SQL. 
    ...
    // Le second appel, $stmtCinema->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);, désactive la fonctionnalité d'émulation de requêtes préparées de PDO. 
    ...
    // C'est pour cela qu'on utilise la méthode prepare().
    // Car c'est ce qui nous permet d'optimiser l'exécution de la requête et de prévenir les attaques par injection SQL.
    
    // On définit une requête SQL qui sélectionne toutes les colonnes de la table movie en utilisant la chaîne de caractères 'SELECT * FROM movie'.
    $stmtCinema = $pdo->query('SELECT movie_title, DATE_FORMAT(release_date, "%Y-%M-%D") AS release_date FROM movie');
    // On récupère donc tout le contenu de la table movie : 

    // La méthode query() est une méthode de l'objet PDO qui permet d'exécuter une requête SQL qui ne requiert pas de préparation préalable. La méthode prend en paramètre la requête SQL à exécuter et renvoie un objet PDOStatement qui peut être utilisé pour récupérer les résultats de la requête.
    ...
    {
        echo '<p>'.$movie['movie_title']." - ".$movie['release_date'].'<br></p>';
    }
