...
<div class="container-fluid d-flex flex-column align-items-center">
    ...
    <form action="index.php?action=addFilm" method="POST">        
        ...
        <div class="form-group mb-1">
            ...
            <select name="director_id" class="form-select form-select-sm" aria-label=".form-select-sm example">
                <option selected> Choose a director </option>
                <?php                
                    foreach($directors as $director)
                    { 
                ?>
                     <option value="<?= $director['director_id'] ?>"> <?= $director['firstName'] . ' ' . $director['lastName'] ?> </option>
                <?php
                    }
                ?>
            </select>
